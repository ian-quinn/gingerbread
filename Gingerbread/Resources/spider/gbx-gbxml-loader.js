...
// See also ISSF file check

GBX.XXXgetXML2jsobj = function( node ) {
...
GBX.getSurfaceMeshes = function() {
	...
	for ( let surface of surfaces ) {
		...
		if ( len <= 3 ) {
			...
			GBX.count3++;
			GBX.setTriangle( surface, vertices, color );

		} else if ( len === 4 && !openings ) {

			GBX.count4++;
			GBX.setQuad( surface, vertices, color );

		} else {

			GBX.count5plus ++;
			GBX.setPolygon( surface, vertices, color, holes );
...
GBX.getVertices = function( polyloop ) {
	...
	const points = polyloop.CartesianPoint.map(
...
GBX.setTriangle = function( surface, vertices, color ) {
	//console.log( 'vertices', vertices );
...
GBX.setQuad = function( surface, vertices, color ){

	const verticesRemix = vertices.slice( 0, 3 ).concat( [ vertices[ 3 ], vertices[ 2 ], vertices[ 0 ] ] );
	GBX.setTriangle( surface, verticesRemix, color );
...
GBX.setPolygon = function( surface, vertices, color, holes = [] ) {
	//console.log( { holes } );
...
GBX.getSurfaceEdgesGbxml = function() {
	...
	for ( let surfaceJson of GBX.gbjson.Campus.Surface ) {
...
GBX.setAllVisible = function() {
	...
	if ( !child.material && !child.userData ) { continue; }
		...
		child.material = new GBX.materialType( {
			color: GBX.colors[ child.userData.gbjson.surfaceType ], side: 2, opacity: 0.85, transparent: true }
		);
